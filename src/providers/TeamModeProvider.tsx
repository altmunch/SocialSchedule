...
interface TeamModeContextType {
  ...
  // Current view state
  currentTab: 'operations' | 'analytics' | 'settings' | 'workflows';
  setCurrentTab: (tab: 'operations' | 'analytics' | 'settings' | 'workflows') => void;
  viewMode: 'grid' | 'list';
...
export function TeamModeProvider({ children }: { children: ReactNode }) {
  ...
  const [currentTab, setCurrentTab] = useState<'operations' | 'analytics' | 'settings' | 'workflows'>('operations');
